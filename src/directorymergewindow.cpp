...
class MergeFileInfos {
    public:
        ...
        bool dirA() const {
            return m_pFileInfoA ? m_pFileInfoA->isDir() : false;
        }
        bool dirB() const {
            return m_pFileInfoB ? m_pFileInfoB->isDir() : false;
        }
        bool dirC() const {
            return m_pFileInfoC ? m_pFileInfoC->isDir() : false;
        }
        bool isLinkA() const {
            return m_pFileInfoA ? m_pFileInfoA->isSymLink() : false;
        }
        bool isLinkB() const {
            return m_pFileInfoB ? m_pFileInfoB->isSymLink() : false;
        }
        bool isLinkC() const {
            return m_pFileInfoC ? m_pFileInfoC->isSymLink() : false;
        }
        bool existsInA() const {
            return m_pFileInfoA != 0;
        }
        bool existsInB() const {
            return m_pFileInfoB != 0;
        }
        bool existsInC() const {
            return m_pFileInfoC != 0;
        }
...
QString DirectoryMergeWindow::getDirNameA() {
    return d->m_dirA.prettyAbsPath();
}
QString DirectoryMergeWindow::getDirNameB() {
    return d->m_dirB.prettyAbsPath();
}
QString DirectoryMergeWindow::getDirNameC() {
    return d->m_dirC.prettyAbsPath();
}
QString DirectoryMergeWindow::getDirNameDest() {
    return d->m_dirDest.prettyAbsPath();
}
        ...
        if( bMergeMode ) {
            ...
            switch( e->key() ) {
                ...
                case Qt::Key_4:
                    ...
                    return;
                case Qt::Key_Delete:
                    if( pMFI->existsInA() && pMFI->existsInB() ) slotCurrentDeleteAAndB();
